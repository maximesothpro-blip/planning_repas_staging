...
app.post('/api/modify-recipe', async (req, res) => {
  try {
    ...
    const dataToSend = {
      ...
      title: recipeData.title,
      description: recipeData.description,
      ingredients: recipeData.ingredients, // Garder le format texte pour modification
      recipe: recipeData.recipe, // Garder le format texte pour modification
      timestamp: new Date().toISOString()
    };

    console.log('Sending modification request to n8n:', dataToSend.title);

    const n8nResponse = await axios.post(N8N_MODIFY_RECIPE_WEBHOOK, dataToSend, {
      timeout: 60000 // 60 secondes pour laisser le temps à n8n de traiter
    });
