...
app.get('/api/shopping-lists', async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/Listes%20de%20Courses`,
      {
        headers: {
          'Authorization': `Bearer ${AIRTABLE_API_KEY}`
        },
...
app.get('/api/shopping-list/:id', async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/Listes%20de%20Courses/${listId}`,
      {
        headers: {
          'Authorization': `Bearer ${AIRTABLE_API_KEY}`
        }
...
app.patch('/api/shopping-list/:id', async (req, res) => {
  try {
    const listId = req.params.id;
    const { ingredients, repasInclus, statut, notes } = req.body;
